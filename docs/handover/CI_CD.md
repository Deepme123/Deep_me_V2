...
### push 이벤트 (`main` / `develop` 브랜치 — 운영/테스트 분리)

`BRANCH_ENV_MAP`(`app/backend/routers/deploy_webhook.py`)이 브랜치를 배포
대상으로 매핑합니다: `main` → 운영(`deep-me-v2`), `develop` → 테스트
(`deep-me-v2-test`, `render.yaml` 참조). 그 외 브랜치로의 push는 무시됩니다.

```
GitHub main/develop push
    │
    ▼
POST /webhook/github  (X-GitHub-Event: push)
    │  서명 검증 (GITHUB_WEBHOOK_SECRET, HMAC-SHA256)
    │  branch가 BRANCH_ENV_MAP에 없으면 무시
    ▼
Render Deploy Hook 호출 (RENDER_DEPLOY_HOOK_URL_PROD 또는 _TEST)
    │
    ├─ RENDER_API_KEY_{PROD|TEST} + RENDER_SERVICE_ID_{PROD|TEST} 설정된 경우
    │     → 최대 10분간 10초 간격으로 배포 상태 폴링
    │     → live/build_failed/update_failed/canceled/deactivated 감지
    │     → 결과를 Discord로 알림 (성공/실패 + 배포 URL, 운영/테스트 채널 분리 가능)
    │
    └─ 미설정인 경우
...
| `RENDER_DEPLOY_HOOK_URL_PROD` / `_TEST` | Render Deploy Hook URL. 미설정 시 해당 환경 배포가 트리거되지 않고 실패 알림만 감 |
| `RENDER_API_KEY_PROD` / `_TEST`, `RENDER_SERVICE_ID_PROD` / `_TEST` | 배포 상태 폴링용 (선택 — 없으면 트리거 성공만 확인) |
| `DISCORD_WEBHOOK_URL_PROD` / `_TEST` | 배포/PR 알림을 받을 채널. `DISCORD_ERROR_WEBHOOK_URL`(에러 로그용)과는 별개 |

---

## 3. 배포 절차

`render.yaml`에 서비스가 두 개 정의되어 있습니다: 프로덕션(`deep-me-v2`,
`main` push로 배포)과 테스트(`deep-me-v2-test`, `develop` push로 배포). 즉
`develop`이 사실상 스테이징 환경 역할을 겸합니다.

실제 흐름 (CLAUDE.md 브랜치 전략과 결합):

1. `feat/*` / `fix/*` 브랜치에서 작업 → `develop`에 push/merge
   - GitHub push 웹훅이 `/webhook/github`을 호출 → 테스트 서비스(`deep-me-v2-test`)로 자동 배포
2. `develop`에서 테스트 서비스로 통합 확인 (자동화된 CI는 없음 — `ci.yml`은 `main` PR에서만 동작)
3. `develop` → `main` PR 생성 → **`ci.yml`이 실행**되어 테스트 + 마이그레이션
   드리프트 검사를 통과해야 함 → 리뷰 필수(CLAUDE.md)
4. PR을 `main`에 merge
   - GitHub push 웹훅이 `/webhook/github`을 호출 → 프로덕션 서비스(`deep-me-v2`)로 Render Deploy Hook 트리거
